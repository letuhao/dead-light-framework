...
| Phase 2 — Codex per Chapter | **First Chapter (Adeptus Administratum) sealed via debate 005 on 2026-05-11; v1.1 implementation-amendment 2026-05-13** (script-invocation references per debate 007 §G; commit `8ebc4da`). Remaining Chapters pending real-project trigger. Codex amendment-procedure precedent established at §10 — implementation amendments don't require new debate; design amendments do. |
...
| [007 — Scripting Infrastructure](framework/debates/007-scripting-infrastructure.md) | Authorise Python tooling under `scripts/` to handle mechanical work (frontmatter validation, link checking, framework/ → distribution/ sync). Tier 1 implemented (validate_frontmatter / check_links / sync_distribution). Library stack: python-frontmatter + typer + markdown-it-py. Dry-run-by-default for writes; strict transactions. Codex §8 step 5 integration deferred to debate 008. Adds **LLM token-cost** as a third design axis (alongside human-time + correctness). ~99% reduction on mechanical work via single-script invocation vs ~50–100K-token manual passes. |
