...
| Scripting infrastructure | **Sealed via debate 007 on 2026-05-13.** Tier 1 scripts implemented at `scripts/` (commit `1246b25`): `validate_frontmatter.py`, `check_links.py`, `sync_distribution.py`. Python 3.10+ with `python-frontmatter` + `typer` + `markdown-it-py`. Dry-run-by-default + `--apply` pattern for writes; strict transactions; severity-graded findings per IVP rubric. **Codex amendment deferred to debate 008** to preserve Codex §10 amendment discipline. Tier 2/3 scripts deferred to subsequent commits per debate 007 §3. |
