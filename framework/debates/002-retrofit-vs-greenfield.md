...
|---|---|---|
| Constitutional law | Japan post-WWII (1947), Spain post-Franco (1978), South Africa post-apartheid (1996) | Retrofit constitutions need an explicit mechanism to handle past violations. South Africa's Truth & Reconciliation Commission is the strongest example. Without such a mechanism the new charter floats over an unaddressed history and erodes. **Selection note:** these three are post-WWII *successful* retrofits. Failed-retrofit cases (Iraq 2005 — legitimacy questioned, sustained insurgency; Weimar Germany 1919 — eventual collapse) suggest the durability anchor is not the existence of a retrofit constitution per se but the presence of *explicit past-violations machinery* — which the TRC has and the Iraq 2005 / Weimar cases conspicuously lack. The framework's Reckoning step is the analogue of TRC machinery for software governance. |
...
| Open source governance | Linux kernel Code of Conduct adoption (2018), Rust Foundation creation (2021), Docker → Moby split | "Effective date" with **de facto** non-retroactive application to prior code and community behavior is the common pattern. Explicit grandfathering policy is rarely written but the absence of retroactive judgment on prior contributions is universally observed in practice; retroactive enforcement of new rules on past behavior is universally rejected and creates lasting damage when attempted. |
