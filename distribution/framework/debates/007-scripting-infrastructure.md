...
- **Decision:** Approved per TL;DR §15 — all nine sub-decisions A–I adopted. Plus one explicit resolution of open question §14 #6:
  - **Open question §14 #6 (Codex amendment via debate 008 vs follow-up of this debate):** **Strict reading adopted.** Codex §10 explicitly says amendments require a new debate "with the same level of rigor as debate 005." Bundling Codex amendments into other debates' follow-up actions would erode this discipline before the first Codex amendment-need is even satisfied. **Therefore: debate 008 will be opened to formally amend Adeptus Administratum Codex v1.0 → v1.1 with script-invocation references.** Until debate 008 seals, Adeptus Administratum instances may invoke scripts informally (as project-owner-authorized one-off task work) but the Codex §8 step 5 will not formally reference scripts.
- **Decided on:** 2026-05-13
- **Decided by:** project owner

### Follow-up actions

- [x] Update [debates/README.md](README.md) — debate 007 marked decided.
- [x] Create `scripts/` directory at repo root with `requirements.txt` and a `README.md` explaining the scripts.
- [x] **Tier 1 implementation:** `scripts/sync_distribution.py`, `scripts/validate_frontmatter.py`, `scripts/check_links.py`.
- [x] Test Tier 1 scripts against current repo state (dry-run validation).
- [ ] **Tier 2 implementation (deferred to subsequent commits):** `snapshot_case_study.py`, `bump_version.py`, `update_handoff_tree.py`, `frontmatter_set.py`.
- [ ] **Tier 3 (deferred indefinitely; build on need):** `release.py` orchestrator, `new_debate.py`, `new_chapter_codex.py`, `new_case_study.py`, `ivp_phase5.py`.
- [ ] **Open debate 008** to amend Adeptus Administratum Codex v1.0 → v1.1 with script-invocation references at §8 step 5 (re-priming step) and §4 Autonomy Threshold (read-only script invocation = solo; write-script invocation = requires acknowledgment).
- [ ] After debate 008 seals: populate adopter-usable subset under `distribution/scripts/` via `sync_distribution.py`.
- [x] Update HANDOFF.md status table (add "Scripting infrastructure" row).
- [ ] Once Codex v1.1 sealed (post-debate-008): update Adeptus Administratum Codex §8 step 5 / §3 / §4 with concrete script references.
